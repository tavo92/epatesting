...
createBoxPlot <- function() {
        for(subj in subjects) {
			...
			if (budget=='600') {
				...
				epatransition_pit = line_branch_exception_epatransition_rows$PIMUT
				epatransition_epaexception_pit = line_branch_exception_epatransition_epaexception_rows$PIMUT
				edges_pit = line_branch_exception_edges_rows$PIMUT
