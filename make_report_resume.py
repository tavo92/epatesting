...
header_names = ['ID', 'BUD', 'SUBJ', 'TOOL', 'LINE', 'BRNCH', 'EPA', 'EXCEP', 'EXCEPTOT', 'ADJAC', 'ADJACTOT', 'ERR', 'NERR', 'TERR', 'MUT', 'TIME', 'LOC', 'PIMUT', 'ERRF', 'MJMUT', 'ERRPROTKILLED', 'ERRPROT', 'ERRNOPROT', 'GENS']
...
def write_row(writer, row):
    writer.writerow({'ID': row[0], 'BUD': row[1], 'SUBJ': row[2], 'TOOL': row[3], 'LINE': row[4], 'BRNCH': row[5], 'EPA': row[6], 'EXCEP': row[7], 'EXCEPTOT': row[8],
                     ...
                     'PIMUT': row[17], 'ERRF': row[18], 'MJMUT': row[19], 'ERRPROTKILLED': row[20], 'ERRPROT': row[21], 'ERRNOPROT': row[22], 'GENS': row[23]});
...
def get_complete_row(row):
    return [row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[9], row[10], 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', row[11],
            'N/A', row[12], row[13], row[14], row[15], row[16]]
...
def report_resume_row(target_class, evosuite, generatios, jacoco, pit, runid, search_budget, criterion, mujava_csv):
    ...
    generations_test = read_generations_csv(generatios)
    ...
    row = [runid, search_budget, target_class, criterion, line_coverage, branch_coverage, epa_coverage, epa_exception, epa_exception_tot, epaadjacentedges, epaadjacentedges_tot, mutation_coverage, mujava_coverage, err_prot_killed, err_prot, err_no_prot_killed, generations_test]
    return row


def make_report_resume(target_class, evosuite, generatios, jacoco, pit, output_file, runid, search_budget, criterion, mujava_csv):
    with open(output_file, 'w', newline='') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=header_names)

        writer.writeheader()
        row = report_resume_row(target_class, evosuite, generatios, jacoco, pit, runid, search_budget, criterion, mujava_csv)
